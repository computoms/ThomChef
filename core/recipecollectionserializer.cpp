...
RecipeCollection RecipeCollectionSerializer::read(std::string filename)
{
    ...
    {
        Recipe recipe = readRecipe(recipeNode);
        recipeCollection.addRecipe(recipe);
    }
    return recipeCollection;
}

Recipe RecipeCollectionSerializer::readRecipe(xml_node recipeNode)
{
...
//    Recipe recipe(name, Conversions::to_int(category), description, ingredients, Conversions::to_bool(isFrozen));
...
bool RecipeCollectionSerializer::serializeTo(std::string filename, RecipeCollection recipeCollection)
{
    xml_document doc;
    xml_node recipeCollectionNode = doc.append_child("RecipeCollection");
    for (int i(0); i < recipeCollection.getNumberOfRecipes(); ++i)
    {
        Recipe recipe = recipeCollection.getRecipe(i);
//        recipeCollectionNode.append_copy(serializeRecipe(recipe));
    }
    return doc.save_file(filename.c_str());
}

pugi::xml_node RecipeCollectionSerializer::serializeRecipe(Recipe recipe)
{
